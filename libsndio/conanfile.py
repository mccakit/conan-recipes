...
class libsndio(ConanFile):
    name = "libsndio"
    version = "master"
    settings = "os", "arch", "compiler", "build_type"
    def source(self):
        subprocess.run(f'bash -c "git clone --recurse-submodules --shallow-submodules --depth 1 git@github.com:ratchov/sndio.git -b {self.version}"', shell=True, check=True)
    def build(self):
        ...
        subprocess.run(f'bash -c "source {autotools_cross} && ./configure --prefix={self.package_folder} {type} && make -j{cpu_count} && make install"', shell=True, check=True)

    def package_info(self):
        self.cpp_info.libs = ["asound", "atopology"]
