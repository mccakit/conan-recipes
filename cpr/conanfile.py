...
class cpr(ConanFile):
    ...
    def source(self):
        subprocess.run(
            f'bash -c "git clone --recurse-submodules --shallow-submodules --depth 1 git@github.com:libcpr/cpr.git -b {self.version}"',
            shell=True,
            check=True,
        )
    ...
    def build(self):
        ...
        subprocess.run(
            f'bash -c "cmake -B build -G Ninja -DCMAKE_PREFIX_PATH=\\"{cmake_prefix_path}\\" -DCMAKE_TOOLCHAIN_FILE={cmake_toolchain} -DCMAKE_INSTALL_PREFIX={self.package_folder} -DCPR_USE_SYSTEM_CURL=ON"',
            shell=True,
            check=True,
        )
        subprocess.run(
