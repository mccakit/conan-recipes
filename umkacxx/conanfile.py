...
class umkacxx(ConanFile):
    name = "umkacxx"
    version = "main"
    settings = "os", "arch", "compiler", "build_type"
    def requirements(self):
        self.requires("umka/pr0")

    def source(self):
        subprocess.run(
            f'bash -c "git clone --recurse-submodules --shallow-submodules --depth 1 git@github.com:mccakit/umkacxx.git -b {self.version}"',
            shell=True,
            check=True,
        )
