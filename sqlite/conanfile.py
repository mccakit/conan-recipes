...
class sqlite(ConanFile):
    ...
    version = "master"
    def source(self):
        subprocess.run(f'bash -c "git clone --recurse-submodules --shallow-submodules --depth 1 git@github.com:sqlite/sqlite.git -b {self.version}"', shell=True, check=True)
    def build(self):
        ...
        subprocess.run(f'bash -c "source {autotools_cross} && ./configure --host={autotools_target} --prefix={self.package_folder} {type} --disable-tcl && make -j{cpu_count} && make install"', shell=True, check=True)
