...
class thread_pool(ConanFile):
    ...
    def source(self):
        sp.run(
            f'bash -c "git clone --recurse-submodules --shallow-submodules --depth 1 git@github.com:mccakit/thread_pool.git -b {self.version}"',
            shell=True,
            check=True,
        )
