...
class lmdbxx(ConanFile):
    name = "lmdbxx"
    version = "main"
    settings = "os", "arch", "compiler", "build_type"

    def requirements(self):
        self.requires("lmdb/main")

    def source(self):
        sp.run(
            f'bash -c "git clone --recurse-submodules --shallow-submodules --depth 1 git@github.com:mccakit/lmdbxx.git -b {self.version}"',
            shell=True,
            check=True,
        )
