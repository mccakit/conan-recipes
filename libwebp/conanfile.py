...
class libwebp(ConanFile):
    name = "libwebp"
    def source(self):
        subprocess.run(f'bash -c "git clone --recurse-submodules --shallow-submodules --depth 1 git@github.com:webmproject/libwebp.git -b {self.version}"', shell=True, check=True)
