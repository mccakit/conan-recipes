...
class simdjson(ConanFile):
    ...
    def build(self):
        ...
        subprocess.run(
            f'bash -c "cmake -B build -G Ninja -DCMAKE_PREFIX_PATH=\\"{cmake_prefix_path}\\" -DCMAKE_TOOLCHAIN_FILE={cmake_toolchain} -DCMAKE_INSTALL_PREFIX={self.package_folder} -DSIMDJSON_DISABLE_DEPRECATED_API=ON"',
            shell=True,
            check=True,
        )
        subprocess.run(
